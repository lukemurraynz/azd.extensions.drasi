...
description: >-
  Performs comprehensive preflight validation of Bicep deployments to Azure, including template syntax validation, what-if analysis, and permission checks. USE FOR: pre-deployment validation before any Azure deployment.
---
...
> [!NOTE]
> `az deployment what-if` can fail with transient network errors. Wrap in a retry loop:
> ```bash
> for i in 1 2 3; do
>   az deployment group what-if --template-file main.bicep --parameters @params.bicepparam && break
>   echo "Attempt $i failed, retrying in 10s..."
>   sleep 10
> done
> ```
> Also: what-if output may include resource property values. Review output before sharing in PR comments to avoid leaking sensitive configuration.
...
- **Authoritative references:** [az deployment what-if](https://learn.microsoft.com/cli/azure/deployment/group#az-deployment-group-what-if), [Bicep CLI reference](https://learn.microsoft.com/azure/azure-resource-manager/bicep/bicep-cli)

### Verification Steps

1. Confirm `az deployment what-if` and `az bicep build` command syntax is current
2. Verify preflight validation scopes (subscription vs resource group) match latest Azure CLI behavior
3. Check for new validation capabilities (e.g., deployment stacks preflight)
