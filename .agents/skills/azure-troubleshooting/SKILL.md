...
description: >-
  Azure resource troubleshooting patterns including KQL templates, metric thresholds, health checks, and remediation playbooks. USE FOR: diagnosing unhealthy Azure resources or building diagnostic workflows AFTER deployment.compatibility: Requires Azure CLI with resource-graph extension
---
...
## ADAC Lens for Troubleshooting (Auto-Detect → Auto-Declare → Auto-Communicate)

When diagnosing issues, evaluate whether the system follows the ADAC triad. Many incidents stem from violations of these principles.

### Common ADAC Violations to Look For

| Violation                        | Symptom                                                  | Fix                                                              |
| -------------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------- |
| Silent catch blocks              | Service appears healthy but returns no data              | Replace with fail-fast or `HealthCheckResult.Degraded("reason")` |
| Health endpoint returns bare 200 | Monitoring says "healthy" during outage                  | Return structured body with dependency status                    |
| No `x-error-code` header         | Consumers can't programmatically distinguish error types | Add RFC 9457 Problem Details with `x-error-code`                 |
| No correlation ID in errors      | Can't trace frontend error to backend logs               | Propagate `traceparent` / `x-correlation-id` end-to-end          |
| Retry without idempotency        | Duplicate operations after transient failures            | Add `Repeatability-Request-ID` or make operation idempotent      |

### Diagnostic Questions (ADAC)

1. **Auto-Detect**: Does the health endpoint detect all critical dependencies (DB, cache, external APIs)?
2. **Auto-Declare**: Does the health endpoint return `Degraded` vs `Unhealthy` with a structured reason?
3. **Auto-Communicate**: Do error responses include `x-error-code`, correlation ID, and actionable detail for consumers?
...
- **Authoritative references:** [Azure Monitor metrics](https://learn.microsoft.com/azure/azure-monitor/essentials/metrics-supported), [KQL reference](https://learn.microsoft.com/kusto/query/)

### Verification Steps

1. Confirm KQL syntax and function names against current reference
2. Verify metric names and thresholds for each resource type are still accurate
3. Check for new Azure Monitor diagnostic categories or deprecated metrics

---

## Related Skills

- [Azure Deployment Preflight](../azure-deployment-preflight/SKILL.md) — Catch issues before deployment
- [Azure Defaults](../azure-defaults/SKILL.md) — Baseline configuration standards
- [Cost Optimization](../cost-optimization/SKILL.md) — Right-sizing and spend analysis
- [WAF Assessment](../waf-assessment/SKILL.md) — Well-Architected review for systemic issues
