...
**Follow the phased deployment approach documented in the sections below (7 phases over 17 weeks)**
...
**Infrastructure templates are provided inline in the deployment phases below**
    ...
    steps:
      ...
      - name: Smoke Test
        run: |
          ...
          curl -f https://management.azure.com/subscriptions/${{ env.AZURE_SUBSCRIPTION_ID }}/resourceGroups/rg-apim-dev-uks/providers/Microsoft.ApiManagement/service/apim-api-marketplace-dev-uks?api-version=2024-05-01 \
...
- **API version used:** `Microsoft.ApiManagement/service@2024-05-01` (GA)
- **Sources:** [APIM ARM reference](https://learn.microsoft.com/azure/templates/microsoft.apimanagement/service), [APIOps Guide](https://learn.microsoft.com/azure/architecture/example-scenario/devops/automated-api-deployments-apiops)
- **Verification steps:**
  1. Verify APIM API version: `az provider show --namespace Microsoft.ApiManagement --query "resourceTypes[?resourceType=='service'].apiVersions" -o tsv`
  2. Check APIOps toolkit version: review [APIOps releases](https://github.com/Azure/apiops/releases)

### Known Pitfalls

| Area                        | Pitfall                                                                                         | Mitigation                                                                                                  |
| --------------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Initial APIM deployment     | First-time APIM provisioning takes 30–45 minutes; CI/CD may timeout                             | Separate infra deployment (day 1) from API config deployment (day 2+); use `--what-if` for previews         |
| Named values with Key Vault | Named values referencing Key Vault secrets require managed identity setup before API deployment | Deploy APIM identity and Key Vault access first; then deploy API configurations that reference named values |
| Extractor tool drift        | APIOps extractor output format changes between toolkit versions                                 | Pin extractor version in CI; test extraction output after toolkit upgrades                                  |
| API version set ordering    | Deploying API version sets out of order causes validation errors                                | Deploy version sets before APIs that reference them; use Bicep `dependsOn` for ordering                     |
| Gateway cache invalidation  | Deployed policy changes may not take effect immediately due to gateway caching                  | Allow 1–2 minutes for cache propagation; test with `Cache-Control: no-cache` header during validation       |
...
**Primary Knowledge**: Phased deployment strategy (inline above), Infrastructure templates (inline above)
