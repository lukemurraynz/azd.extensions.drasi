...
- [ ] T022 [TEST] [US5] Write validate command test: exit 0 on valid fixture, exit 1 on error fixture, exit 2 on missing drasi.yaml, --strict promotes warnings to errors, `--environment <name>` is accepted and passed through, --output json valid schema — `cmd/validate_test.go`
...
- [ ] T046 [P] [US2] Create FederatedIdentityCredential module: resource `FederatedIdentityCredential` on UAMI, subject `system:serviceaccount:${drasiNamespace}:drasi-resource-provider`, audience `api://AzureADTokenExchange`, OIDC issuer from AKS output — `infra/modules/fedcred.bicep`
...
- [ ] T067 [TEST] [P] [US3] Write teardown command test: missing --force exits 2 with ERR_FORCE_REQUIRED; --infrastructure flag accepted; `--environment <name>` accepted; `--output json` emits structured deletion summary; components deleted in reverse order reactions→middleware→queries→sources — `cmd/teardown_test.go`
...
- [ ] T084 [TEST] [P] [US4] Write logs command test: --component filter accepted, --kind filter accepted, --tail n flag wired, --follow flag accepted, `--environment <name>` accepted, `--output json` emits NDJSON in follow mode (no mixed human text), pod not found exits 1 — `cmd/logs_test.go`
...
- [ ] T088 [P] [US4] Implement `cmd/logs.go`: shell out to `kubectl logs` for Drasi pods in drasiNamespace; support `--environment <name>` context resolution; filter by `--component` (pod label match) or `--kind`; `--tail n` passes to kubectl; `--follow` passes -f to kubectl; `--output json` emits NDJSON (one JSON log object per line) without human formatting — `cmd/logs.go`
- [ ] T089 [P] [US4] Implement `cmd/diagnose.go`: run 5 checks sequentially — (1) AKS API server reachable via kubectl, (2) Drasi control-plane/resource-provider workload reachable in drasiNamespace, (3) Dapr sidecar injector pod running, (4) Secrets Store CSI sync health for generated SecretProviderClass objects in namespace, (5) Log Analytics last-5-min ingestion present; support `--environment <name>`; accumulate PASS/FAIL; exit 1 if any fail — `cmd/diagnose.go`
...
| Phase 3: US5 Validate  | P5    | T016–T031                             | 11        | 7          |
| Phase 4: US1 Scaffold  | P1    | T032–T040, T104–T105                  | 5         | 3          |
| Phase 5: US2 Provision | P2    | T041–T053                             | 9         | 1          |
| Phase 6: US3 Deploy    | P3    | T054–T082, T102–T103, T109–T110, T112 | 20        | 17         |
