...
- **FR-024**: `azd drasi provision` MUST deploy: AKS cluster, Log Analytics workspace, Azure Key Vault, Managed Identity with the following least-privilege role assignments (per Principle IV, NON-NEGOTIABLE): **`Key Vault Secrets User`** on the Key Vault (enables Drasi components to read secrets at runtime); **`Monitoring Metrics Publisher`** on the Log Analytics workspace (enables OpenTelemetry metric export per FR-038); **`AcrPull`** on the container registry (only when a private ACR hosts Drasi component images — omitted if public images are used). No broader roles (e.g., `Contributor`, `Key Vault Secrets Officer`) are permitted. Optionally: Cosmos DB account, Event Hubs namespace, and Service Bus namespace when declared in the active configuration.
