...
- **FR-045**: `azd drasi provision` MUST configure AKS Workload Identity for the Drasi runtime namespace. This requires: (1) the AKS cluster provisioned with `--enable-oidc-issuer` and `--enable-workload-identity`; (2) a `FederatedIdentityCredential` linking the UAMI to the Drasi Kubernetes ServiceAccount (`system:serviceaccount:drasi-system:drasi-api`) using the cluster's OIDC issuer URL and audience `api://AzureADTokenExchange`. The namespace `drasi-system` is the Drasi-installed default; a Bicep parameter `drasiNamespace` (default `drasi-system`) MUST be provided to support custom namespace deployments via `drasi init -n <namespace>`. (3) the Drasi Kubernetes ServiceAccount annotated with `azure.workload.identity/client-id: <UAMI_CLIENT_ID>`; (4) Drasi runtime pods labelled with `azure.workload.identity/use: "true"`. All four steps MUST be applied via Bicep; no manual `az identity federated-credential create` steps are permitted.
  [VERIFY] EvidenceType=Code | WhereToCheck=https://github.com/drasi-project/drasi-platform/tree/main/installer — confirm the ServiceAccount name `drasi-api` matches the one created by `drasi init` in the `drasi-system` namespace before finalizing the Bicep FederatedIdentityCredential subject.
...
- **FR-046**: The extension MUST invoke the Drasi CLI (`drasi`) as a subprocess for all Drasi runtime operations. The extension MUST NOT re-implement the Drasi API client. The following CLI commands are the complete set used by the extension:
...
- **FR-036**: `azd drasi init` MUST generate a local development compose configuration that starts an optional local Drasi server and Dapr sidecar for offline query testing. The Drasi server container image MUST use `ghcr.io/drasi-project/drasi:<version>` where `<version>` defaults to `0.10.0` and is overridable via a `DRASI_VERSION` environment variable in the generated compose file.
  [VERIFY] EvidenceType=Registry | WhereToCheck=https://github.com/drasi-project/drasi-platform/pkgs/container — confirm exact image name and available tags before generating the compose template at implementation time.
...
- **Drasi CLI coexistence**: The `drasi` CLI (minimum version **0.10.0**) is available alongside `azd` in the Dev Container. The azd extension invokes Drasi CLI commands for all runtime operations (apply, wait, delete, list, describe) as specified in FR-046; it does not re-implement the Drasi API client from scratch. The CLI binary is resolved from `PATH` at runtime; absence causes a fail-fast `ERR_DRASI_CLI_NOT_FOUND` error. ContinuousQuery resources use the canonical kind string `continuousquery` in all CLI invocations.
...
- Note (user-identified gap): Drasi runtime pods require AKS Workload Identity to authenticate to Azure services without stored credentials. Added FR-045: AKS OIDC issuer, Workload Identity addon, `FederatedIdentityCredential`, ServiceAccount annotation (`azure.workload.identity/client-id`), and pod label (`azure.workload.identity/use: "true"`) — all provisioned via Bicep. FR-043 updated to mandate Workload Identity authentication for Key Vault reads during secret translation.
...
- Q: What is the canonical Drasi Kubernetes ServiceAccount identity for the FR-045 FederatedIdentityCredential subject? → A: **`system:serviceaccount:drasi-system:drasi-api`** (`drasi-system` is the default namespace from `drasi init`; add Bicep parameter `drasiNamespace` for custom namespace scenarios). Added [VERIFY] block — confirm SA name `drasi-api` from drasi-platform installer source before Bicep authoring.
...
- Q: What Docker image reference should the FR-036 local compose use for the optional local Drasi server? → A: **`ghcr.io/drasi-project/drasi:<version>`** with `DRASI_VERSION` env var (default `0.10.0`). Added [VERIFY] block — confirm exact image name from ghcr.io/drasi-project packages before generating compose template.
