...
- **FR-047**: All mutating commands (`provision`, `deploy`, `teardown`) MUST emit a structured audit event for each mutation attempt. At minimum the event MUST include: `operation`, `environment`, `correlationId`, `target`, `result`, `startedAtUtc`, and `endedAtUtc`. In `--output json` mode, audit events MUST be emitted as JSON with no mixed human-formatted text.
...
- **FR-046**: The extension MUST invoke the Drasi CLI (`drasi`) as a subprocess for install/bootstrap and Drasi runtime operations. The extension MUST NOT re-implement the Drasi API client. The only Drasi CLI commands used by the extension are:
  ...
  - **FR-048**: Provisioning MUST ensure every generated or patched Kubernetes workload in scope of the Drasi runtime has `livenessProbe`, `readinessProbe`, and `startupProbe` definitions. Missing probe configuration on any generated workload is a deployment defect.
  - **FR-049**: `azd drasi provision` MUST detect existing resource group resources that are not tagged `managed-by=azd`, MUST NOT modify or delete those resources, and MUST emit a warning list of unmanaged resources in both human-readable and `--output json` modes.
...
- **FR-038**: `azd drasi provision` MUST configure all AKS-hosted Drasi components to export OpenTelemetry traces and metrics to the Log Analytics workspace. This includes configuring the deployed Drasi runtime workloads (not only the extension process) with exporter settings and validating those settings through `azd drasi diagnose` checks.
...
- **FR-040**: `azd drasi logs --component <id>` MUST stream structured log lines from the named Drasi component in real time by default; `--tail <n>` MUST limit output to the last n historical lines on initial connection before resuming the live stream. When `--tail` is omitted, the command MUST stream from the current position forward only (live tail; no historical lines are fetched on initial connection). If a `--follow` flag is exposed, it MUST be treated as a compatibility alias and MUST NOT change behavior (streaming is already the default). When `--output json` is set, log output MUST be emitted as JSON objects (NDJSON for streaming mode), with no mixed human-formatted text.
