...
Conventions for Rust development targeting the 2021 edition. The 2024 edition is available since Rust 1.85 (February 2025); these conventions are compatible with both editions unless noted otherwise.

**IMPORTANT**: Use the `iseplaybook` MCP server for ISE Rust and engineering best practices. Use `context7` MCP server for crate documentation and API verification. Use `microsoft.learn.mcp` for Azure SDK for Rust guidance. Do not assume crate APIs are stable across versions — verify.
...
For multi-crate projects, use a Cargo workspace with resolver v2:
...
crates/
  ...
  shared-tests/         # Common test fixtures
```
...
Member crates inherit shared versions and lints:
...
- Use `[workspace.dependencies]` to centralize versions across all member crates.
- Use `[workspace.lints]` to enforce consistent clippy and compiler lint levels.
- Member crates opt in with `.workspace = true` syntax.
- Keep workspace roots thin; avoid placing library code at the workspace root.
...
All public types should derive `Debug` at minimum. Eagerly derive standard traits when the type supports them:

- `Debug` — required for all public types (Rust API Guideline C-DEBUG).
- `Clone` — derive unless the type manages unique resources.
- `PartialEq`, `Eq` — derive when equality comparison is meaningful.
- `Hash` — derive when the type may be used as a map key or set element.
- `Default` — derive or implement when a zero/empty value is sensible.
...
Use bounded channels for backpressure between async producers and consumers:
...
- Prefer `mpsc::channel` (bounded) over `mpsc::unbounded_channel` to prevent unbounded memory growth.
- Use `oneshot::channel` for single request-response exchanges between tasks.
- Use `broadcast::channel` when multiple consumers need the same messages.
- Use `watch::channel` for single-value state sharing (e.g., configuration updates).
- Consider `tokio-stream` for stream combinators on channel receivers.

### Async Trait Implementations

Native `async fn` in traits is stable since Rust 1.75 and works with all editions (including 2021). Use native async traits when concrete types are sufficient:
...
Native async traits do not support `dyn Trait` dispatch. Use `async-trait` when you need dynamic dispatch (`Box<dyn Repository>`) — it boxes the returned future and adds `Send` bounds automatically:
...
For plugin architectures, implement a blanket delegation from `Box<dyn Trait>` to the inner type:
...
impl Repository for Box<dyn Repository> {
    ...
    async fn save(&self, item: &Item) -> Result<()> {
        self.as_ref().save(item).await
    }
}
```

This enables callers to store `Box<dyn Repository>` and call methods without unwrapping. Use this when components are loaded dynamically (e.g., plugin systems via `libloading`).
...
When distributed tracing is required, integrate OpenTelemetry via `tracing-opentelemetry`. Add the following to `[dependencies]`:
...
fn init_tracing() {
    ...
    if env::var("OTEL_EXPORTER_OTLP_ENDPOINT").is_ok() {
        let exporter = SpanExporter::builder()
            .with_tonic()
            .build()
            .expect("OTLP exporter must initialize at startup");
...
Use `tonic` + `prost` for gRPC services. Organize proto files under a dedicated directory:
...
build.rs
src/
  generated/          # Output from prost-build (gitignored or committed)
```
...
- Pin tonic and prost to compatible minor versions (see Known Pitfalls). Since tonic 0.14, prost codegen is in separate `tonic-prost` / `tonic-prost-build` crates — the core `tonic-build` re-exports still work for standard usage.
- Use `build.rs` to compile `.proto` files; avoid manual codegen.
- Prefer `tonic::Status` for gRPC error responses; map domain errors to appropriate gRPC status codes.
...
- **Date checked:** 2026-07-22 (verified via docs.rs, GitHub releases, and Rust API Guidelines)
- **Rust edition:** 2021 (2024 edition available since Rust 1.85, February 2025)
- **Compatibility:**
  - tokio 1.x, serde 1.x, thiserror 2.x, tracing 0.1.x, tracing-subscriber 0.3.x
  - tonic 0.14.x, prost 0.14.x, prost-types 0.14.x, tonic-build 0.14.x
  - opentelemetry/opentelemetry-sdk/opentelemetry-otlp 0.31.x, tracing-opentelemetry 0.32.x
  - mockall 0.13.x, wiremock 0.6.x, reqwest 0.13.x
- **MSRV:** OpenTelemetry crates require Rust 1.75+
- **Verification steps:**
  1. `cargo clippy --all-targets --all-features` exits cleanly
  2. `cargo test` passes
  3. `cargo build --release` produces a working binary
  4. Check crate versions: `cargo search <crate-name>`

## Known Pitfalls

| Area                     | Pitfall                                                                                                                                      | Mitigation                                                                                                                                                                                               |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| async-trait vs native    | Native `async fn` in traits (Rust 1.75+) does not support `dyn Trait` dispatch                                                               | Use `async-trait` crate when `Box<dyn Trait>` or `&dyn Trait` is needed                                                                                                                                  |
| OpenTelemetry API churn  | OTel Rust SDK has frequent breaking changes across minor versions (the `new_pipeline()` API was removed in 0.27+)                            | Pin the full OTel stack to the same minor version; check the [CHANGELOG](https://github.com/open-telemetry/opentelemetry-rust/blob/main/opentelemetry/CHANGELOG.md) before upgrading                     |
| tokio-retry maintenance  | `tokio-retry` 0.3.0 last published ~2020; functional but unmaintained                                                                        | Consider `backon` crate as an actively maintained alternative for new projects                                                                                                                           |
| `unwrap()` in async init | `expect()` in OTel/tracing init is acceptable at startup but panics if called after the service accepts traffic                              | Restrict `expect()` to `main()` and `OnceLock`/`LazyLock` initializers only                                                                                                                              |
| Edition migration        | Rust 2024 edition changes `gen` keyword reservation and `unsafe extern` blocks                                                               | Run `cargo fix --edition` before switching; review the [edition guide](https://doc.rust-lang.org/edition-guide/rust-2024/)                                                                               |
| reqwest 0.13 migration   | reqwest 0.13 (Dec 2025) defaults to rustls instead of native-tls; `query`/`form` features now opt-in                                         | Explicitly enable `features = ["json"]` (still required); if you need native-tls, add `features = ["native-tls"]`; review [v0.13 changelog](https://github.com/seanmonstar/reqwest/releases/tag/v0.13.0) |
| tonic/prost coupling     | tonic and prost have tightly coupled version requirements; since tonic 0.14, prost codegen moved to `tonic-prost`/`tonic-prost-build` crates | Always upgrade tonic, prost, prost-types, and tonic-build together; check [tonic release notes](https://github.com/hyperium/tonic/releases) for the required prost version                               |
