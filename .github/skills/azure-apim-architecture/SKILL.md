...
description: >-
  Analyzes and explains Azure API Management architecture decisions for enterprise API marketplace implementations using VNet Internal mode, Front Door, hybrid authentication, and multi-environment strategies. USE FOR: APIM component selection, network topology, cost optimization, or comparing alternatives (workspaces vs instances, VNet modes, Front Door vs Application Gateway).
license: MIT
metadata:
...
**Microsoft Guidance**:
> "Workspaces enable federated API management with segregated administrative permissions and segregated API runtime, while preserving centralized governance in a shared API Management service." - [APIM Workspaces](https://learn.microsoft.com/azure/api-management/workspaces-overview)
...
*Workspaces share same Premium instance cost (£1,944/month). Workspace gateways can isolate runtime, but teams still share one APIM service boundary and service-level governance.
...
- Runtime isolation via dedicated workspace gateways when needed
- Not a full substitute for dev/test/prod separation when you need separate APIM instances, subscriptions, or lifecycle boundaries
...
> [!IMPORTANT]
> **Credential lifecycle management**: Subscription keys should be rotated every 90 days. Configure automatic key rotation via Azure Key Vault integration. For OAuth tokens, ensure token refresh is handled by the client library (MSAL handles this automatically). Never embed subscription keys in source code — inject via Named Values backed by Key Vault references.
...
> **Pricing snapshot note (2026-03-19):** Values below are estimate snapshots for planning, not fixed rates. Always re-validate in the Azure pricing calculator before approvals or production changes.
...
- **Compatibility:** APIM Premium/Premium v2 with workspace gateways and centralized platform governance
- **Sources:** [Workspaces overview](https://learn.microsoft.com/azure/api-management/workspaces-overview), [API Management gateways overview](https://learn.microsoft.com/azure/api-management/api-management-gateways-overview), [API Management pricing](https://azure.microsoft.com/pricing/details/api-management/)
- **Verification steps:**
  1. Re-check workspace runtime/isolation guidance in the Workspaces overview before changing environment boundaries.
  2. Re-check gateway limits/constraints for workspace gateways and update architecture notes if constraints change.
  3. Re-validate all GBP monthly estimates with the Azure pricing calculator before production budget sign-off.

### Known Pitfalls

| Area | Pitfall | Mitigation |
|------|---------|------------|
| Workspace isolation assumptions | Treating workspaces as a full environment-isolation substitute can hide shared-service risks. | Use separate APIM instances for hard dev/test/prod boundaries; use workspace gateways for team/runtime segmentation inside an environment. |
| Workspace gateway placement | Putting too many critical workspaces on one shared gateway increases noisy-neighbor risk. | Assign dedicated gateways for mission-critical workloads; distribute non-critical workspaces across multiple gateways. |
| Static pricing guidance | Hardcoded cost values drift quickly as SKU pricing and usage assumptions change. | Treat listed amounts as reference only and require a fresh calculator run per release. |
