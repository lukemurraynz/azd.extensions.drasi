...
description: >-
  Creates production-ready Azure API Management policy XML for authentication (OAuth 2.0, JWT validation, subscription keys), rate limiting, CORS configuration, error handling, and API transformations. USE FOR: implementing API security, access control, or request/response processing logic within APIM policies.license: MIT
metadata:
...
> [!IMPORTANT]
> Policy templates in this skill provide structural patterns. Before production use, add retry policies (`<retry>`) for backend calls, circuit breaker logic via `<cache-lookup>` + error tracking, and `<timeout>` on `<send-request>` elements. Without these, transient backend failures cascade to all API consumers.
...
- **ADAC**: Backend failures return structured reason, no silent swallowing

---

## ADAC Resilience at the Gateway (Auto-Detect → Auto-Declare → Auto-Communicate)

API Management sits between consumers and backends — it MUST follow the ADAC triad. See `csharp.instructions.md` for the backend ADAC pattern.

### Auto-Detect

- Detect backend timeouts and 5xx responses in `<on-error>`.
- Detect rate-limit exhaustion before forwarding (existing `rate-limit-by-key` policy).
- Detect authentication failures (expired/invalid JWT) early in `<inbound>`.

### Auto-Declare

- Return structured error responses with `correlationId`, `errorCode`, and `message` — never a raw 502/504 from the backend.
- Set `x-error-code` header on all error responses so consumers can programmatically react.
- Declare backend health status in response headers when operating in degraded mode.

### Auto-Communicate

- When a backend is unreachable, return `503 Service Unavailable` with a structured body that includes the reason (timeout, circuit open, dependency down).
- Include `Retry-After` header on 429 and 503 responses.
- Never silently swallow backend errors — if the gateway intercepts a failure, the consumer must know.
...
- **Authoritative references:** [APIM Policy Reference](https://learn.microsoft.com/azure/api-management/api-management-policies), [Microsoft.ApiManagement ARM templates](https://learn.microsoft.com/azure/templates/microsoft.apimanagement/service)

### Verification Steps

1. Confirm latest GA API version for `Microsoft.ApiManagement/service` (currently `2024-05-01`)
2. Verify policy syntax against [Policy Reference](https://learn.microsoft.com/azure/api-management/api-management-policies)
3. Check for deprecated policies or renamed attributes in release notes
