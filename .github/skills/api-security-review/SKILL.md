...
> [!NOTE]
> MCP server references (e.g., `microsoft.learn.mcp`, `iseplaybook`) require the corresponding MCP server to be configured in the Copilot runtime. If unavailable, use direct documentation links as fallback: https://learn.microsoft.com/azure/api-management/api-management-policies
...
**Check**: Inbound webhooks validate payload integrity using HMAC signatures

**Implementation Pattern**:

```csharp
// Verify HMAC-SHA256 signature on inbound webhook
app.MapPost("/webhooks/provider", async (HttpRequest request) =>
{
    ...
    var secret = config["Webhook:Secret"]; // from Key Vault
...
- Always use `CryptographicOperations.FixedTimeEquals` (constant-time comparison) — never `==` or `string.Equals` to prevent timing attacks
- Store the webhook secret in Key Vault, never in app settings or code
- Return `401` for missing signatures, `403` for invalid signatures
- Enable request buffering if the body is consumed multiple times

**Risk if Missing**: Payload tampering, replay attacks, spoofed webhook events
...
- **Compatibility:** ASP.NET Core 10, Azure API Management, any HTTP API framework
- **OWASP API Security Top 10:** 2023 edition (released — replaces 2019 edition)
- **OWASP Top 10 (Web):** 2025 edition (released — replaces 2021 edition). See [OWASP Top 10:2025](https://owasp.org/Top10/2025/)
- **OWASP Top 10 for LLM/GenAI:** 2025 edition. See [LLM Top 10](https://genai.owasp.org/llm-top-10/) — relevant for AI-backed API endpoints and agent-facing APIs
- **Sources:** [OWASP API Security Top 10 (2023)](https://owasp.org/API-Security/editions/2023/en/0x00-header/), [OWASP Top 10:2025](https://owasp.org/Top10/2025/), [OWASP LLM Top 10 (2025)](https://genai.owasp.org/llm-top-10/), [Azure Security Benchmark](https://learn.microsoft.com/security/benchmark/azure/overview), [APIM Security Best Practices](https://learn.microsoft.com/azure/api-management/security-controls-policy)
- **Verification steps:**
  1. Verify OWASP API Security edition: check [OWASP API Security project page](https://owasp.org/API-Security/) for latest edition
  2. Verify OWASP Top 10 (Web) edition: check [OWASP Top 10 project page](https://owasp.org/www-project-top-ten/) for latest edition
  3. Verify OWASP LLM Top 10 edition: check [GenAI Security project page](https://genai.owasp.org/llm-top-10/) for latest edition
  4. Verify APIM policy schema: `az apim api operation list --help` for current CLI surface
  5. Review Azure Security Benchmark controls: [Azure Security Benchmark v3](https://learn.microsoft.com/security/benchmark/azure/overview)

### Known Pitfalls

| Area                        | Pitfall                                                                                         | Mitigation                                                                                        |
| --------------------------- | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| BOLA (API1:2023)            | Missing object-level authorization lets users access other users' resources via ID manipulation | Enforce ownership checks on every data-access path, not just controller-level `[Authorize]`       |
| JWT validation              | Validating signature but not `aud`, `iss`, or `exp` claims allows token reuse across services   | Validate all claims including audience, issuer, and expiration; reject tokens with missing claims |
| Rate limiting bypass        | In-memory rate limiting resets on pod restart; per-pod limits don't aggregate in multi-replica  | Use distributed rate limiting (Redis-backed) or API Management rate-limit policies                |
| CORS wildcards              | `Access-Control-Allow-Origin: *` in production bypasses browser same-origin protections         | Explicitly list allowed origins; never use wildcard in production                                 |
| Mass assignment (API3:2023) | Binding request body directly to domain models exposes internal fields to modification          | Use dedicated DTOs/request models; never bind directly to entity types                            |
| Middleware ordering         | `UseAuthentication()` placed after `UseAuthorization()` silently bypasses authentication        | Always place `UseAuthentication()` before `UseAuthorization()` in ASP.NET Core pipeline           |
| SSRF (API7:2023)            | Accepting user-supplied URLs without validation enables server-side request forgery             | Validate and allowlist URL schemes/hosts; reject private IP ranges and internal hostnames         |
