...
description: >-
  Analyze Azure architectures for cost optimization opportunities, provide savings recommendations, and calculate ROI for improvements. USE FOR: reviewing architecture costs, identifying waste, right-sizing, reserved instances.
---
...
## AKS Cost Defaults (AKS Book)

- Prefer a **single public entrypoint** (Gateway API/Ingress) instead of per-service LoadBalancers.
- Use **AKS Application Routing add-on** unless you need WAF/advanced features (then AGC).
- Use **Managed Prometheus** for metrics and **Container Insights logs/events only** to avoid duplicate metric costs.
- Use **NAT Gateway** for outbound; add **Cilium** only if you need L7 policies; avoid Azure Firewall unless required.
- Use **reserved instances** for system and baseline node pools; **spot** for dev/test or interruptible batch jobs.
- Track 30-day utilization; target **60–70% average** and right-size VM SKUs.
- Set autoscaler **min** to baseline (often 3 nodes), **max** with ~20% buffer; scale-down delay 5–10 minutes.
- Enable **AKS Cost Analysis add-on** + budgets and run monthly orphaned resource audits.
...
- **Authoritative references:** [Azure Advisor cost recommendations](https://learn.microsoft.com/azure/advisor/advisor-cost-recommendations), [Azure pricing calculator](https://azure.microsoft.com/pricing/calculator/)

### Verification Steps

1. Confirm Azure Advisor cost recommendation categories are still current
2. Verify reservation and savings plan term options (1-year, 3-year) for referenced SKUs
3. Check for new cost optimization features (e.g., new Advisor categories, Azure Cost Management updates)

---

## Related Skills

- [WAF Assessment](../waf-assessment/SKILL.md) — Full Well-Architected review including Cost Optimization pillar
- [Azure Defaults](../azure-defaults/SKILL.md) — Default SKU selections and resource baselines
- [Azure Troubleshooting](../azure-troubleshooting/SKILL.md) — Performance diagnostics before right-sizing
